...
| :---     | :--- | :---: | :--- | :--- |
| Cluster  | `report_cluster.sql` | PG >= 10 | **Full Cluster Audit**: Generates a comprehensive Markdown report including config, connections, replication, and more. | Multiple |
| Database | `report_database.sql` | PG >= 10 | **Full Database Audit**: Generates a detailed report of the current database including sizes, bloating, and index health. | Multiple |
...
| Database | `extensions.sql` | PG >= 9.0 | List all installed extensions and their versions | `pg_extension` |
...
| Cluster | `statements_group_database_resume.sql` | PG >= 14 | top 10 statements on all cluster order by execution time | `pg_stat_statements` |
...
| Cluster | `statements_group_database_total.sql` | PG >= 14 | total statements summary on all cluster | `pg_stat_statements` |
