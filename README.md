...
- **📊 Smart Reporting**: Detailed reports that clearly show object details, performance insights, slow queries and more
...
| :---     | :---                              | :---: 		   | :---                                          | :---      |
| Cluster  | `access_method.sql`               | PG >= 9.6     | Access methods (index types) available on the cluster | [`pg_am`](https://www.postgresql.org/docs/current/catalog-pg-am.html) |
...
| Database | `collations.sql`                  | PG >= 10      | Collation inventory                           | [`pg_collation`](https://www.postgresql.org/docs/current/catalog-pg-collation.html) |
| Database | `conversions.sql`                 | PG >= 8.2     | Encoding conversion inventory                 | [`pg_conversion`](https://www.postgresql.org/docs/current/catalog-pg-conversion.html) |
| Database | `data_types.sql`                  | PG >= 8.3     | Custom data type inventory                    | [`pg_type`](https://www.postgresql.org/docs/current/catalog-pg-type.html) |
| Database | `domains.sql`                     | PG >= 8.2     | Domain inventory                              | [`pg_type`](https://www.postgresql.org/docs/current/catalog-pg-type.html), [`pg_constraint`](https://www.postgresql.org/docs/current/catalog-pg-constraint.html) |
...
| Database | `foreign_servers.sql`             | PG >= 8.4     | Foreign servers and their user mappings       | [`pg_foreign_server`](https://www.postgresql.org/docs/current/catalog-pg-foreign-server.html), [`pg_user_mappings`](https://www.postgresql.org/docs/current/view-pg-user-mappings.html) |
| Database | `large_objects.sql`               | PG >= 12      | Large objects (BLOB) inventory                | [`pg_largeobject_metadata`](https://www.postgresql.org/docs/current/catalog-pg-largeobject-metadata.html) |
| Database | `operators.sql`                   | PG >= 8.2     | Operator inventory                            | [`pg_operator`](https://www.postgresql.org/docs/current/catalog-pg-operator.html) |
| Database | `procedural_languages.sql`        | PG >= 8.3     | Procedural languages installed                | [`pg_language`](https://www.postgresql.org/docs/current/catalog-pg-language.html) |
...
| Database | `tables_partition.sql`            | PG >= 10      | Partitioned tables: size, partition count and cache hit rate per parent table | [`pg_partitioned_table`](https://www.postgresql.org/docs/current/catalog-pg-partitioned-table.html), [`pg_partition_tree()`](https://www.postgresql.org/docs/current/functions-info.html#FUNCTIONS-INFO-PARTITION) function |
| Database | `tables_partition_top5.sql`       | PG >= 10      | Top 5 largest partitions per partitioned table, with size and cache hit rate | [`pg_partitioned_table`](https://www.postgresql.org/docs/current/catalog-pg-partitioned-table.html), [`pg_partition_tree()`](https://www.postgresql.org/docs/current/functions-info.html#FUNCTIONS-INFO-PARTITION) function |
...
| Database | `tables_constraint_validate.sql`        | PG >= 9.1     | VALIDATE CONSTRAINT command for every constraint marked as `NOT VALID`                                   | [`pg_constraint`](https://www.postgresql.org/docs/current/catalog-pg-constraint.html)                                                                                 | VALIDATE CONSTRAINT takes only a `SHARE UPDATE EXCLUSIVE` lock but still scans the whole table |
...
| Database | `index_poor_rep.sql`                    |               | Indexes with bad performance, combining usage stats from more than one replica                           | [`pg_stat_user_indexes`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-INDEXES-VIEW)                                           | experimental — requires setting up cross-replica stats beforehand |
...
| Both     | `stats_reset_all.sql`                   | PG >= 9.0     | Reset all stats from current database and cluster                                                        | [`pg_stat_reset_*`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-STATS-FUNCS-TABLE) functions                                                        | This script run an `ANALYZE` command on current database to prevent autovacuum issues |
| Both     | `stats_last_reset.sql`                   | PG >= 9.0     | Show last reset from current database and cluster                                                       | [`pg_stat_reset_*`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-STATS-FUNCS-TABLE) functions                                                        | This script run an `ANALYZE` command on current database to prevent autovacuum issues |
...
| Both     | `vacuum_wraparound.sql`                 |               | Runs the full wraparound report (database, table, multixact age and clean-up commands, in sequence)      | [`pg_database`](https://www.postgresql.org/docs/current/catalog-pg-database.html), [`pg_class`](https://www.postgresql.org/docs/current/catalog-pg-class.html)       | |
...
<details>
<summary>🔄 Replication (Logical & Physical)</summary>

| Scope    | Name                                | Compatibility | Description                                              | Reference |
| :---     | :---                                | :---:         | :---                                                      | :---      |
| Database | `publications.sql`                  | PG >= 10      | Publications inventory                                   | [`pg_publication`](https://www.postgresql.org/docs/current/catalog-pg-publication.html) |
| Database | `publication_schemas.sql`           | PG >= 15      | Schemas included in each publication                     | [`pg_publication_namespace`](https://www.postgresql.org/docs/current/catalog-pg-publication-namespace.html) |
| Database | `publication_tables.sql`            | PG >= 12      | Tables included in each publication                      | [`pg_publication_tables`](https://www.postgresql.org/docs/current/view-pg-publication-tables.html) |
| Database | `replication_origin.sql`            | PG >= 9.5     | Logical replication origin status                        | [`pg_replication_origin`](https://www.postgresql.org/docs/current/catalog-pg-replication-origin.html) |
| Cluster  | `replication_slots.sql`             | PG >= 9.4     | Replication slots inventory                               | [`pg_replication_slots`](https://www.postgresql.org/docs/current/view-pg-replication-slots.html) |
| Cluster  | `replication_slots_logical.sql`     | PG >= 14      | Logical replication slot statistics                       | [`pg_stat_replication_slots`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-REPLICATION-SLOTS-VIEW) |
| Cluster  | `replication_stats.sql`             | PG >= 9.1     | Physical replication stats per standby                    | [`pg_stat_replication`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-REPLICATION-VIEW) |
| Database | `subscription_stats.sql`            | PG >= 10      | Subscription status and stats                             | [`pg_subscription`](https://www.postgresql.org/docs/current/catalog-pg-subscription.html), [`pg_stat_subscription`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-SUBSCRIPTION-VIEW) |
| Database | `subscription_table_stats.sql`      | PG >= 10      | Per-table initial sync status for each subscription        | [`pg_subscription_rel`](https://www.postgresql.org/docs/current/catalog-pg-subscription-rel.html) |
...
| Scope    | Name                                               | Compatibility | Description                                                                                  | Reference                                                                                                                           |Comments |
| :---     | :---                                               | :---:         | :---                                                                                         | :---                                                                                                                                | :---    |
| Cluster  | `clean_query.sql`                                  |               | Ask for a Query ID and show the query without line breaks and spaces                         | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html)                                                 | Only works on psql |
| Cluster  | `connections_blocking_vacuum.sql`                  | PG >= 9.4     | Connections that may be blocking VACUUM cleanup                                              | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
| Cluster  | `connections_by_client_addr.sql`                   | PG >= 9.2     | Active connections stats by client address:                                                   | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
| Cluster  | `connections_by_database.sql`                      | PG >= 9.2     | Active connections stats by database                                                         | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
...
| Cluster  | `connections_by_wait_event.sql`                    | PG >= 9.2     | Active connections stats by wait event                                                       | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
...
| Cluster  | `connections_running_detail.sql`                   |               | Active detailed connections stats running now                                                | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
| Cluster  | `connections_by_app_name.sql`                      | PG >= 9.2     | Active connections stats by application name                                                 | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
...
| Cluster  | `connections_io.sql`                               | PG >= 18      | Active connections with I/O and WAL throughput per second per backend, top 20 by I/O bytes/s | [`pg_stat_get_backend_io()`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-STATS-FUNCTIONS) / [`pg_stat_get_backend_wal()`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-STATS-FUNCTIONS) functions | |
...
| Database | `kill_active_wait_events.sql`                      | PG >= 9.6     | Kill active sessions that have any wait event type except "Client"                           | [`pg_stat_activity`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW)                | |
...
| Cluster  | `progress_analyze.sql`                             | PG >= 13      | Status about ANALYZE commands running                                                        | [`pg_stat_progress_analyze`](https://www.postgresql.org/docs/current/progress-reporting.html#ANALYZE-PROGRESS-REPORTING)            | |
...
| Cluster  | `vacuum_blocker.sql`                               | PG >= 9.4     | List what may be blocking VACUUM cleanup | [`pg_stat_activity`](https://www.postgresql.org/docs/18/monitoring-stats.html#MONITORING-PG-STAT-ACTIVITY-VIEW), [`pg_prepared_xacts`](https://www.postgresql.org/docs/current/view-pg-prepared-xacts.html), [`pg_replication_slots`](https://www.postgresql.org/docs/current/view-pg-replication-slots.html), [`pg_stat_replication`](https://www.postgresql.org/docs/18/monitoring-stats.html#MONITORING-PG-STAT-REPLICATION-VIEW) | |
...
| Database | `autovacuum_analyze_duration.sql` | PG >= 18     | Top 20 tables by ANALYZE + autoanalyze time, with count/day, time/day and avg time per run                            | [`pg_stat_user_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW)        | |
...
| Database | `autovacuum_vacuum_plus.sql`     | PG >= 9.2     | Top 20 tables with more percentage of dead rows with separate values for toast tables                                  | [`pg_stat_all_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW)         | experimental |
...
| Database | `autovacuum_vacuum_adjust_plus.sql` | PG >= 8.4  | ALTER TABLE command to adjust autovacuum_vacuum_scale factor based on table size with separate values for toast tables | [`pg_stat_all_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW)         | experimental |
| Database | `autovacuum_vacuum_duration.sql` | PG >= 18      | Top 20 tables by VACUUM + autovacuum time, with count/day, time/day, avg time per run and cost delay/limit             | [`pg_stat_user_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW)        | |
...
| Database | `functions_stats.sql`            | PG >= 8.4     | Top 20 functions by self time, with calls/day and average time — used internally by `report_database.sql` | [`pg_stat_user_functions`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-USER-FUNCTIONS-VIEW) | |
...
| Database | `autovacuum_vacuum_queue.sql`           |               | Next tables where autovacuum will work                      | [`pg_stat_user_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-ALL-TABLES-VIEW) |
| Cluster  | `conf_auth.sql`                         | PG >= 8.4     | Authentication related parameters                            | [`pg_settings`](https://www.postgresql.org/docs/current/view-pg-settings.html) |
| Cluster  | `conf_ssl.sql`                          | PG >= 8.4     | SSL related parameters                                       | [`pg_settings`](https://www.postgresql.org/docs/current/view-pg-settings.html) |
| Cluster  | `shared_buffers_stats.sql`              | PG >= 13      | Shared buffers allocation breakdown                          | [`pg_shmem_allocations`](https://www.postgresql.org/docs/current/view-pg-shmem-allocations.html) |
| Cluster  | `slru_stats.sql`                        | PG >= 13      | SLRU cache statistics                                        | [`pg_stat_slru`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-SLRU-VIEW) |
...
| Cluster  | `conf_logs_adjust.sql`                  |               | Set parameters about logs configurations and other related  | [`pg_settings`](https://www.postgresql.org/docs/current/view-pg-settings.html) |
...
| Database | `io_table_tidx.sql`                     | PG >= 9.1     | I/O stats for TOAST indexes on tables                            | [`pg_statio_all_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STATIO-ALL-TABLES-VIEW) |
| Database | `io_table_toast.sql`                    | PG >= 9.1     | I/O stats for TOAST tables                                       | [`pg_statio_all_tables`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STATIO-ALL-TABLES-VIEW) |
| Cluster  | `io_cluster_write.sql`                  | PG >= 16      | Cluster-wide I/O write statistics                                | [`pg_stat_io`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-IO-VIEW) |
| Database | `statements.sql`                        | PG >= 8.4     | Runs the full pg_stat_statements report (all `statements_*` sections in sequence) | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
...
| Cluster  | `statements_cluster.sql`                | PG >= 8.4     | Runs the full cluster-wide pg_stat_statements report (all `statements_cluster_*` sections in sequence) | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_summary.sql`        | PG >= 14      | top 10 statements on all cluster order by execution time        | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_temp.sql`           | PG >= 9.2     | top 10 statements on all cluster order by temporary files       | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_time.sql`           | PG >= 9.4     | top 10 statements on all cluster order by execution time        | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_total.sql`          | PG >= 14      | total statements summary on all cluster                         | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_total_by_database.sql` | PG >= 14   | total statements summary on all cluster grouped by database     | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Cluster  | `statements_cluster_total_by_user.sql`  | PG >= 14      | total statements summary on all cluster grouped by user          | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Database | `statements_exec.sql`                   | PG >= 8.4     | top 10 statements order by execution time                       | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
...
| Database | `statements_plan.sql`                   | PG >= 14      | top 10 statements planning time                                 | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Database | `statements_summary.sql`                 | PG >= 14      | top 20 statements summary order by planning and execution time | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
| Database | `statements_summary_group.sql`           | PG >= 14      | statements summary grouped by user and queryid                 | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
...
| Database | `statements_top5.sql`                   | PG >= 8.4     | top 5 full query statements order by execution and planning time | [`pg_stat_statements`](https://www.postgresql.org/docs/current/pgstatstatements.html) extension |
...
| Cluster  | `connections_ssl.sql`                   | PG >= 9.5     | total connections stats running now using SSL                               | [`pg_stat_ssl`](https://www.postgresql.org/docs/current/monitoring-stats.html#MONITORING-PG-STAT-SSL-VIEW)                                                                                                                   | |
...
| Database | `revoke_from_pg_catalog_functions.sql`  | PG >= 9.1     | Generates REVOKE commands for non-default grants on pg_catalog functions     | [`pg_proc`](https://www.postgresql.org/docs/current/catalog-pg-proc.html)                                                                                                                                                    | |
| Cluster  | `role_migrate_paas.sql`                 | PG >= 8.4     | Generates commands to migrate roles and their memberships from one cluster to another | [`pg_roles`](https://www.postgresql.org/docs/current/catalog-pg-roles.html), [`pg_auth_members`](https://www.postgresql.org/docs/current/catalog-pg-auth-members.html) | useful when the target cluster is a managed/PaaS service without superuser access |
...
| Cluster  | `user_priv.sql`                         |               | roles with high privilege options like superusers                           | [`pg_roles`](https://www.postgresql.org/docs/current/catalog-pg-roles.html)                                                                                                                                                  | |
...
- **Be careful using kill scripts on a troubleshooting session**: These scripts will kill sessions on the current cluster. Be careful before playing with these tools.
