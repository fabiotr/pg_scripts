SELECT
    ...
    pg_size_pretty(trunc((current_setting('block_size')::bigint * blks_hit)  / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)))::bigint) AS "Hit/Day",
    pg_size_pretty(trunc((current_setting('block_size')::bigint * blks_read) / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)))::bigint) AS "Read/Day",
	...
	trim(to_char(checksum_failures::NUMERIC / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)), '999G999D9')) END   AS "Checksum fail / Day",
    checksum_last_failure 														  AS "Last Checksum fail",
    CASE temp_files WHEN 0 THEN NULL ELSE 
	trim(to_char(temp_files::NUMERIC / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'999G990D9')) END 	  AS "Temp file  / Day",
    CASE temp_files WHEN 0 THEN NULL ELSE 
	trim(pg_size_pretty(temp_bytes   / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)))) END 	                  AS "Temp bytes / Day",
    ...
    '------------' 															  AS "Sessions",
    trim(to_char(sessions::NUMERIC           / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'999G990D9')) 	  AS "Total / Day",
    trim(to_char(sessions_abandoned::NUMERIC / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'999G990D9')) 	  AS "Abandoned / Day",
    trim(to_char(sessions_fatal::NUMERIC     / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'999G990D9')) 	  AS "Fatal / Day",
    trim(to_char(sessions_killed::NUMERIC    / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'999G990D9')) 	  AS "Killed / Day",
    date_trunc('second',(session_time / sessions)   						 		      * INTERVAL '1 MILLISECOND') AS "Time / Session",
    date_trunc('second',active_time              / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)) * INTERVAL '1 MIlLISECOND') AS "Active / Day",
    date_trunc('second',idle_in_transaction_time / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)) * INTERVAL '1 MIlLISECOND') AS "Idle in Trans. / Day", 
    '------------' 															  AS "Reset",
