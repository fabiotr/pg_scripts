...
| assesment        | cluster  | `security_labes.sql`                   | PG >= 9.1     | security labels on SE Linux Security Policies | `pg_seclabels` | |
| assesment        | database | `security_policies.sql`                | PG >= 9.5     | policies for Row Level Security | `pg_policies` | |
...
- Based on pg_stat_wal_receiverBased on pg_stat_wal_receiver
