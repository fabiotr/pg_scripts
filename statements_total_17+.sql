SELECT
    ...
    to_char((sum(total_plan_time) / reset_days) * INTERVAL '1 millisecond' / (sum(calls)/reset_days),'FF6') AS "Avg Plan time (uS)", 
    to_char((sum(total_exec_time) / reset_days) * INTERVAL '1 millisecond' / (sum(calls)/reset_days),'FF6') AS "Avg Exec time (uS)", 
    trunc(sum(total_plan_time)::numeric * 100 / sum(total_plan_time + total_exec_time)::numeric,1) || ' %' AS "Plan %",
    to_char(sum(calls)::numeric / reset_days,'999G999G999') AS "Total calls/Day",
    CASE WHEN current_setting('pg_stat_statements.track') = 'all' 
	THEN to_char(count(1) FILTER (WHERE toplevel = FALSE) / reset_days,'999G999G999')  ELSE 'Disabled' END AS "Non Toplevel calls/Day", 
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
        ...
        ELSE 'Disabled' END AS "Shared time/Day",
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
    ...
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
        ...
        ELSE 'Disabled' END AS "Temp   time/Day",
    to_char((sum(total_plan_time)                   / reset_days) * INTERVAL '1 millisecond', 'HH24:MI:SS') AS "Plan time/Day",
    to_char((sum(total_exec_time)                   / reset_days) * INTERVAL '1 millisecond', 'HH24:MI:SS') AS "Exec time/Day",
    to_char((sum(total_plan_time + total_exec_time) / reset_days) * INTERVAL '1 millisecond', 'HH24:MI:SS') AS "Total time/Day",
    trunc(sum(shared_blks_hit) * 100 / sum(shared_blks_hit + shared_blks_read),1) || ' %' AS "Shared Hit",
    pg_size_pretty((nullif((sum(shared_blks_hit))::numeric,0)     / reset_days) * current_setting('block_size')::integer) AS "Shared Hit/Day",
    pg_size_pretty((nullif((sum(shared_blks_read))::numeric,0)    / reset_days) * current_setting('block_size')::integer) AS "Shared Read/Day",
    pg_size_pretty((nullif((sum(shared_blks_written))::numeric,0) / reset_days) * current_setting('block_size')::integer) AS "Shared Write/Day",
    pg_size_pretty((nullif((sum(shared_blks_dirtied))::numeric,0) / reset_days) * current_setting('block_size')::integer) AS "Shared Dirty/Day",
