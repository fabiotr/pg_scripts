SELECT schemaname, relname, 
    ...
    pg_size_pretty(trunc((current_setting('block_size')::bigint * toast_blks_hit)  / reset_days)::bigint) AS "Toast Hit/Day",
    pg_size_pretty(trunc((current_setting('block_size')::bigint * toast_blks_read) / reset_days)::bigint) AS "Toast Read/Day",
    ...
    to_char(CASE toast_blks_hit WHEN 0 THEN 0 ELSE 100 * toast_blks_hit::NUMERIC / (toast_blks_hit + toast_blks_read) END,'FM990D99') AS "Toast hit %",
    pg_size_pretty(trunc((current_setting('block_size')::bigint * tidx_blks_hit)  / reset_days)::bigint) AS "Tidx Hit/Day",
    pg_size_pretty(trunc((current_setting('block_size')::bigint * tidx_blks_read) / reset_days)::bigint) AS "Tidx Read/Day",
    ...
    to_char(CASE tidx_blks_hit  WHEN 0 THEN 0 ELSE 100 * tidx_blks_hit::NUMERIC  / (tidx_blks_hit  + tidx_blks_read)  END,'FM990D99') AS "Tidx  hit %"
FROM 
