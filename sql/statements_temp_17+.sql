SELECT
    ...
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE 
        THEN ((shared_blk_read_time + shared_blk_write_time + local_blk_read_time + local_blk_write_time + temp_blk_read_time + temp_blk_write_time)/reset_days) * INTERVAL '1 millisecond'
...
ORDER BY (coalesce(temp_blks_read,0) + coalesce(temp_blks_written,0))/since_days DESg
LIMIT 10;
