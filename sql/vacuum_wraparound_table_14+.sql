SELECT
    n.nspname AS "Schema", 
    c.relname AS "Table",
    trim(to_char(age(c.relfrozenxid),'FM999G999G999G999')) ||
        CASE 
            WHEN t.relfrozenxid IS NOT NULL AND c.relfrozenxid != t.relfrozenxid
                THEN ' (T ' || trim(to_char(age(t.relfrozenxid),'FM999G999G999G999')) || ')'
    ...
    trim(to_char(to_number(COALESCE(fma.option_value, current_setting('autovacuum_freeze_max_age')), '999999999999'),'FM999G999G999G999')) ||
            CASE WHEN 
            ...
            ELSE '' END AS "Max Age", 
    trim(to_char(to_number(current_setting('vacuum_failsafe_age'),'999999999999'),'FM999G999G999G999')) AS "Failsafe Age", 
