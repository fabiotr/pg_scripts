SELECT 
    ...
    CASE 
        ...
        WHEN i.current_age < 1600000000 AND coalesce(current_age_toast,0) < 1600000000                            THEN '🟠 Warning'
        ELSE                                                                                                           '🔴 Critic'
        END AS "Status"
FROM (
    SELECT
        n.nspname,
        c.relname,
        mxid_age(c.relminmxid) AS current_age,
        CASE WHEN t.relminmxid IS NOT NULL AND c.relminmxid::text != t.relminmxid::text
                THEN mxid_age(t.relminmxid)
                ELSE NULL END AS current_age_toast,
        LEAST (to_number(COALESCE(fmi.option_value, current_setting('vacuum_multixact_freeze_min_age')),'999999999999'),
            to_number(COALESCE(fma.option_value, current_setting('autovacuum_multixact_freeze_max_age')),'999999999999') /2) AS min_age,
        CASE WHEN
        ...
        LEAST (to_number(COALESCE(ftb.option_value, current_setting('vacuum_multixact_freeze_table_age')),'999999999999'),
            to_number(COALESCE(fma.option_value, current_setting('autovacuum_multixact_freeze_max_age')),'999999999999') * '0.95') AS table_age,
        CASE WHEN
        ...
        to_number(COALESCE(fma.option_value, current_setting('autovacuum_multixact_freeze_max_age')),'999999999999') AS max_age,
        CASE WHEN
    ...
    WHERE
        ...
        (mxid_age(c.relminmxid) >= to_number(COALESCE(fmi.option_value,  current_setting('vacuum_multixact_freeze_min_age')),'999999999999') /2 OR
         mxid_age(t.relminmxid) >= to_number(COALESCE(tfmi.option_value, current_setting('vacuum_multixact_freeze_min_age')),'999999999999') /2)
