SELECT
    n.nspname AS "Schema", 
    c.relname AS "Table",
    to_char(mxid_age(c.relminmxid),'FM999G999G999G999') ||
        CASE 
            WHEN t.relfrozenxid IS NOT NULL AND c.relminmxid != t.relminmxid 
                THEN ' (T ' || to_char(mxid_age(t.relminmxid),'FM999G999G999G999') || ')'
            ELSE '' END AS "Current Age",
    to_char(LEAST (to_number(COALESCE(fmi.option_value, current_setting('vacuum_multixact_freeze_min_age')),'999999999999'),
        to_number(COALESCE(fma.option_value, current_setting('autovacuum_freeze_max_age')),'999999999999') /2),'FM999G999G999G999') || 
            ...
            ELSE '' END AS "Min Age", 
    to_char(LEAST (to_number(COALESCE(ftb.option_value, current_setting('vacuum_multixact_freeze_table_age')),'999999999999'),
    ...
    to_char(to_number(COALESCE(fma.option_value, current_setting('autovacuum_multixact_freeze_max_age')), '999999999999'),'FM999G999G999G999') ||
            CASE WHEN 
            ...
            ELSE '' END AS "Max Age", 
    to_char(to_number(current_setting('vacuum_multixact_failsafe_age'),'999999999999'),'FM999G999G999G999') AS "Failsafe Age", 
