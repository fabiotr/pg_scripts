SELECT
    ...
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
        ...
        ELSE NULL END AS "Read/Day",
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
        THEN to_char((shared_blk_write_time             / since_days) * INTERVAL '1 millisecond', 'HH24:MI:SS')
        ELSE NULL END AS "Write/Day",
    CASE WHEN current_setting('track_io_timing')::BOOLEAN = TRUE
...
ORDER BY (coalesce(shared_blks_read,0) + coalesce(shared_blks_written,0))/since_days DESC
LIMIT 10;
