...
SELECT
    ...
    lpad(pg_size_pretty(pg_relation_size(c.oid, 'main')),7)  AS size
FROM 
    (SELECT 
            (100*1024*1024) / pg_relation_size(oid,'main')::NUMERIC scale, -- 100*1024*1024 = 100MB goal
...
WHERE
    c.relkind IN ('r', 'm', 'p','t') AND  -- Only tables
