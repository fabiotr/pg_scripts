SELECT
    ...
    CASE temp_files WHEN 0 THEN NULL ELSE trim(to_char(temp_files::NUMERIC / reset_days,'FM999G990D9')) END                   AS "Temp file  / Day",
    ...
    CASE deadlocks  WHEN 0 THEN NULL ELSE trim(to_char(deadlocks::NUMERIC  / reset_days,'FM999G990D9')) END                   AS "Deadlocks  / Day",
    CASE checksum_failures WHEN 0 THEN NULL ELSE trim(to_char(checksum_failures::NUMERIC / reset_days, 'FM999G999D9')) END    AS "Checksum fail / Day",
    checksum_last_failure                                                                                                     AS "Last Checksum fail",
    '------------'                                                                                                            AS "Sessions",
    trim(to_char(sessions::NUMERIC           / reset_days,'FM999G990D9'))                                                     AS "Total / Day",
    trim(to_char(sessions_abandoned::NUMERIC / reset_days,'FM999G990D9'))                                                     AS "Abandoned / Day",
    trim(to_char(sessions_fatal::NUMERIC     / reset_days,'FM999G990D9'))                                                     AS "Fatal / Day",
    trim(to_char(sessions_killed::NUMERIC    / reset_days,'FM999G990D9'))                                                     AS "Killed / Day",
    to_char((parallel_workers_to_launch::numeric / reset_days::numeric), 'FM999G999G999')                                     AS "Workers Planned / Day",
    to_char((parallel_workers_launched::numeric  / reset_days::numeric), 'FM999G999G999')                                     AS "Workers Lunched / Day",
