...
FROM 
    ...
    LEFT JOIN (SELECT usename, count(1) AS qt FROM pg_stat_activity WHERE pid != pg_backend_pid() AND usename NOT IN ('rds_superuser', 'rdsadmin') GROUP by usename ORDER BY usename) AS c
