SELECT 
    ...
    lpad(pg_size_pretty(round(coalesce(sum(writes)  * op_bytes/reset_days, 0))),8)      AS "Write/Day",
    lpad(pg_size_pretty(round(coalesce(sum(extends) * op_bytes/reset_days, 0))),8)      AS "Extend size",
...
WHERE 
    writes > 0 AND
    i.object = 'relation' 
GROUP BY i.backend_type, op_bytes, reset_days, stats_reset, t_writes
