SELECT
    ...
    CASE deadlocks  WHEN 0 THEN NULL ELSE trim(to_char(deadlocks::NUMERIC  / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'FM999G990D9')) END AS "Deadlocks  / Day",
    CASE temp_files WHEN 0 THEN NULL ELSE trim(to_char(temp_files::NUMERIC / (EXTRACT(EPOCH FROM current_timestamp - stats_reset) / (60*60*24)),'FM999G990D9')) END AS "Temp file  / Day",
