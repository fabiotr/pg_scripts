SELECT                                  
    ...
    to_char(wal_bytes * 100 / sum(wal_bytes) OVER (),'FM99D99') || '%' AS "WAL %",
    --datname AS "DB",
    ...
    to_char(rows::numeric         / calls::numeric,      'FM999G999G990D9') AS "Rows/Call",
    to_char(wal_records::numeric  / calls::numeric, 'FM999G990D9')          AS "Wal/Call",
    pg_size_pretty(trunc(nullif(wal_bytes::numeric/calls,0)))             AS "WAL Size/Call",
    to_char(wal_records::numeric/reset_days, 'FM999G999G999')               AS "WAL Records/Day",
    to_char(wal_fpi::numeric/reset_days,     'FM9G999G999')                 AS "WAL FPI/Day", 
