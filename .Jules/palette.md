...
**Action:** Wrap generated SQL or CLI command outputs in Markdown code blocks (```sql or ```bash) only when they are meant to be displayed as raw text or executable code; avoid code blocks for outputs that are already formatted as Markdown tables to ensure correct rendering.
